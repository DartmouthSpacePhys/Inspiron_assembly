...
version:
	...
	.word	0D0Ah
	.pstring "SLAVE, 23 December 2009 "
help:
	.word	0D0Ah
...
; 
; Date        Version/Action
; 23 Dec 2009 S11: Changed NCO center frequency to 2.67 MHz per Dartmouth
;                  request.
; 25 Mar 2009 S10: As received from Dartmouth on 23 December 2009.
; 30 Oct 2006 S09: Pulled in state machine code from DARTM09.ASM- but note
;                  that this code is mostly bypassed for the Slave. Updated
;                  comments.
; 29 Oct 2006 S08: (Slave only) Reset TLM FIFO before header transmission.
;                  Commented out unused interrupt code. Minor comment updates.
; 29 Oct 2006 S07: Updated interrupt state machine; added State 2 code
;                  to wait until end of sequence before going back
;                  to look at TLM FIFO empty flag in state 0.
;                  (Slave does not use state machine).
;                  Removed one NOP from rx_loop timing.
;                  Commented out unused interrupt code. 
; 29 Oct 2006 S06: Cleared registers after call to rx_setup, in order to
;                  have major frame and sequence counters start at 0.
;                  Adjusted rx_loop time delay. Made same as Master; consequence
;                  is that acq_seq_out pulse is narrow, turned off at lsb_sel.
; 29 Oct 2006 S05: Slave version.
...
acq_seq_len .set 1600   ; For possible on Slave, number of words for acq_seq high
...
;
;M	cmpm	AR3,#1		; State 1: TLM FIFO found empty, so
...
;
	...
	ldm	AR0,A
	and	#acq_seq_in,A	; Mask to acquiring sequence input from Master
	stlm	A,AR0
	cmpm	AR0,#acq_seq_in	; See if Master has set acq_seq
				; If zero, wait 
...
;
	stm	#acq_seq_out, AR0	; Discrete outputs: lsb_sel low, acq_seq_out high
	portw	AR0,wr_disc	;
...
;M	stm	#(lsb_sel+acq_seq_out), AR0	; [2] Master: lsb_sel, acq_seq_out high
	stm	#(lsb_sel), AR0	; [2] Slave: lsb_sel high
...
static_header:
;M	.pstring	"Dartmouth College Master RxDSP  "
...
ad6620_msg1:
	.word	0D0Ah
;M	.pstring "AD6620 set-up: MASTER "
      .pstring "AD6620 set-up: SLAVE  "
	.word	0D0Ah,0000h
ad6620_msg2:
	.word	0D0Ah
...
ad6620_filter:
;M      .word    0300H, 00000H, 00000H, 00900H	; Soft reset, Master
        .word    0300h, 00000h, 00000h, 00100h  ; Soft reset, Slave
...
;M      .word    0300H, 00000H, 00000H, 00900H	; Soft Reset, Master
        .word    0300H, 00000H, 00000H, 00100H	; Soft Reset, Slave
;M      .word    0300H, 00000H, 00000H, 00800H	; Run: end of soft reset, Master
