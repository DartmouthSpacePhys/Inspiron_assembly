...
;
; Timer control constants (for serial monitor only)
;
...
version:
	...
	.word	0D0Ah
	.pstring "Version 1.12, Dartmouth South-Pole QDSP"
	.word	0D0Ah
	.pstring "MASTER, 6 October 2011"
	.word	0D0Ah
help:
	.word	0D0Ah
...
; 
; Date        Version/Action
; 06 Oct 2011 M12: Modifications to allow semi-continuous operation with 
;                  a Bitwise QuickUSB reading data off the front-end.
;                  Insert an end-of-Rx-FIFO sync word to correct for possible
;                  error where the first word out of the Rx FIFO is not an I.
...
;
	...
	stm		#(int_0+int_1),IMR	; CHARM: Enable MAJOR interrupts
	rsbx	INTM		; Enable interrupts
;  
...
; (24 total words)
;
; After header, 200 words with of I/Q data, with LSB for I = 1, Q = 0, then 
; data with true LSB starts, continuing established I/Q alternating pattern.
;

;;
; Reset FIFOs, send acquire sequence
;;
...
major_loop:
	...
	cmpm	AR3,#128			; AR3 == 0, wait for Tx FIFO, go to serial monitor
	bc		serial_skip,NTC

tlm_fifo_wait:
	nop
	portr	rd_disc, AR3		; Get discrete bits to AR3
	bitf	AR3,#tlm_efo		; Tx FIFO empty?
	bc		tlm_fifo_wait,NTC	; If not, wait for the last minor frame
	b		mon ; go to serial monitor
	
serial_skip:
	cmpm	AR3,#0				 ; if AR3 == 0, we are starting a new major frame
	bc		continue_major,NTC 	 ; otherwise, continue with the current one
...
;
	ldm	AR7,A		; Get MS word of minor frame count
	stlm	A,AR0
	portw	AR0,wr_out	; Write to telemetry FIFO
	ldm	AR6,A		; Get LS word of minor frame count
	stlm	A,AR0
	portw	AR0,wr_out	; Write to telemetry FIFO
...
; End of header, start taking major frame data

	stm		AR3, #1 ; state 1, now working within a major frame
	stm		#lsb_sel, AR0 ; set lsb_sel, unset acq_seq_out
	portw	AR0, wr_disc
	
continue_major:

	stm	#0,AR2		; Zero minor frame word count
...
rx_ready_wait:
	...
	bitf	*AR0, #rx_efo		 ; see if Rx FIFO is empty
	bc		rx_ready_wait, TC	 ; if so, try again
	...
	cmpm	AR2,#iq_len          ; [2] Send first iq_len words with I/Q bit
	bc acqlsb_skip,NTC
	stm		#0, AR0				 ; reset acq_seq_out and lsb_sel
	portw	AR0,wr_disc          ; [2+7] LSB_SEL = 1 => true LS bit
	
acqlsb_skip:
;
; Check # of words sent, end minor frame if max
;
	cmpm	AR2,#minor_len	;[2] Compare to max word count for sequence
	bc		minor_loop,NTC ;[5 true (typ) 3 false] 

;
; Minor frame counter (32 bits, using AR6 and AR7)
;
...
skip_ms:
;
; Strobe watchdog- once per minor frame
...
;
	...
	bd		major_loop		; Delayed branch, followed by 2 nops,
...
major_int:
	stm		#0,AR3 ; set state 0, new major frame
	nop
	rete
; major_int done

minor_int:
	stm		#128,AR3 ; set state 128, go to serial monitor
	nop
	rete
; minor_int done
