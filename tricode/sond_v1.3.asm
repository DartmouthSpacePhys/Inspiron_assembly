...
;
; History
; v1.2	2012.10.21	Added toggle of 
; v1.1	2012.06.01	Added 1-frequency debug mode controlled by trm_28 jumper
; v1.0				First production code
...
cfreq_test		.word   0x0e66, 0x6758	; 3750 KHz, used when trm_28 jumper is on

cfreq_toggle1	.set	0x0343	; 850 kHz
cfreq_toggle2	.set 	0x95B8
...
; first antenna state load	
	mvdm	@ant_state, AR0
	orm		#lsb_sel, AR0	; Reset
	portw	AR0, wr_disc
	nop
...
; original site for cfreq_walk, moving to put the initial setup before 
; the boot delay loop, then all subsequent to after header writing
;	call	cfreq_walk
...
bridge_data:
	...
	mvdm	@ant_state, AR0
	orm		#(lsb_sel), AR0 ; send acq_seq, set lsb_sel
	portw	AR0, wr_disc
...
dinner_is_ready:
	...
	; now that the header is written, we'll set the frequencies up for the next MajFr
	; note cfreq_walk uses A, B, AR0, and AR4, so we need to save what we're using
	
	stlm	B, AR1	; save B
	pshd	AR4		; and AR4
	call	cfreq_walk
	popd	AR4		; restore AR4
	ldm		AR1, B	; and B
...
header_skip:
	...
	stlm	A, AR2	; store a copy of the data to AR2
...
*
****
	.global cfreq_walk, cfreq_commit
cfreq_walk:
...
cfreq_commit:
	...
	stm		#acq_ant_1, AR0		; enable antenna 1
	
	; frequency is in A.  subtract toggle freq and save result to B
	sub		#cfreq_toggle1, #16, A, B
	sub		#cfreq_toggle2, B
	
	xc		2, BGEQ					; if B>=0, cfreq >= toggle freq
	stm		#acq_ant_2, AR0		; so enable antenna 2
	nop
	nop
	nop
...
ready_loop:
	...
	mvdm	@ant_state, AR0
	orm		#(acq_seq_out+acq_seq_rdy+lsb_sel), AR0 ; send acq_seq_out
	portw	AR0, wr_disc
