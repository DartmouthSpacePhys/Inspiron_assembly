...
transfer_sum_loop:
	...
	sub		#1, A
	stl		A, *(Delta)	; store delta-1 for rpt
	
	ld		#0, B
	rpt		*(Delta)		; repeat delta times
	add		*AR4+, B
	
	; now we need to divide/right shift by 2^(# data bits in Delta)
	; so div by 1 for Delta=1, 2 for Delta=2, 4 for 3 or 4, 8 for 5,6,7,8, etc.
	exp		A			; T = # sign bits in A (Delta)
	ld		#31, A
	sub		@T, A		; A = 32-T = # data bits in Delta
	neg		A			; neg => right shift
	nop

	ld		@AL, ASM		; store to Accumulator Shift Mode register
	nop

	ld		B, ASM, B	; do the shift, i.e. divide by 2^ASM
	nop
