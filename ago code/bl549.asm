...
* * * * * * * * * * * * * * * * * * * * * * * * *
	...
	.global	BOOTLOAD_START, blskipskip, xfr08, par08_1, endboot
	.ref	RXDSP_START
	.sect	".bl549"
entry_point		.set	RXDSP_START
eprom_base		.set	0x8000
bl_loadpoint	.set	BOOTLOAD_START
...
****************************************************************************
	...
	bc	endboot,aeq 		; section dest = 0 indicates boot end
	...
	cmpm	AR2, #bl_loadpoint	; check if our dest is the bootloader load address
	bc	blskipskip, NTC			; if not, keep loading
	
	add		#1, A		; if it is the bootloader, we want to skip 
	stlm 	A, AR0		; this section, i.e. skip A+1 words
	nop
	add		AR0, A		; but wait,
	stlm	A, AR0		; A+1 words = 2(A+1) addresses (8-bit prom)
	nop
	bd		par08_1		
	mar 	*AR1+0
	nop
