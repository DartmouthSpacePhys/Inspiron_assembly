...
;
; Revision History redacted
; 
; Date		Monitor Version/Action
; 22 Oct 2011   2.03: Set monitor for 19200 baud rate, trim timer_int to be 
;			shorter, so hopefully less interference with monitor codes.
; 15 Oct 2011	2.02: Dynamic switch in appcode to go to serial monitor.
;			Timer is set up w/ timer interrupt enabled in serial monitor to 
;			strobe the watchdog, so it can always be enabled.
;			Added X command to monitor menu, to dynamically run appcode.
;
...
nbits   .set    8 
nstop   .set    2 
fullbit .set    4167		; Number of CLKOUT1 cycles/bit at 2400 baud, 
halfbit .set    2083		; with 40 MHz TCLK
;
...
mon:
	...
	stm	#int_t,IMR	; Enable timer int
...
main:
	...
	stlm	A,AR7
	sub		#0058h,A,A		; X... eXecute appcode?
	cc		appcode,AEQ
	ldm AR7,A
...
;	stm	#0,state	; Clear interrupt routine state
	...
	stm		#(int_0+int_3),IMR	; Select interrupts to use
;	rsbx	INTM		; Unmask interrupts
	
	;
	; Set up timer for the Rx FIFO RCLK (haaaaack)
	; Can't work with RCLK grounded (WHY?  STUPID!)
	;
	
	stm	#ntss,TCR	; Stop timer, if running
;	stm	#6,PRD	; Load timer period register (7 CPU clocks per trigger)
;	stm	#nload,TCR	; Reset and start timer
...
major_loop:
	...
	; Stop acquisition, reset Rx FIFO, then idle until an interrupt.
...
frame_wait:
	...
	ldm		IFR, A ; manual reset on int_3
	stlm	A, AR0
	bitf	AR0, #int_3
	bc		read_init, TC

	ldm		BSPCE0, A ; store control extension register
	stlm	A, AR2	
	
	stm		#1, AR7 ; set burst counter
	...
	portr	rd_disc, AR1	; Get discrete bits
	bitf	AR1, trm_28		; Test for high terminal input	
	bc		mon, TC			; Branch to serial monitor if high 
...
wait_hf_loop:
	...
	mar		*AR2+
	nop ; delayed branch instrs (mar+nop) will run before next portr
...
;	
	...
	bitf	AR2, #bspce_xh ; check if first or second half of buffer just finished
...
abu_first_half:
	...
	stlm	A, AR3 ; put half address into AR3
	
;
; Math loop: square modulus, add, 10 log_10
;
...
; Strobe watchdog- once per sequence
	...
	; If a burst is complete, go back to IDLE
	b		major_loop
