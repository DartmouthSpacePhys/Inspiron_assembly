...
data_n			.set	128	; Size of each FFT (# of IQ pairs)
data_2n			.set	256	; Size of each FFT (# of words)
data_minor_sz	.set	1 		; number of acquisitions per half-buffer interrupt
abu_buff_loc	.set	0800h	; serial buffer start location
abu_buff_sz		.set	1032	; size of serial buffer (2x minor size IN BYTES)
abu_buff_hsz	.set	516		; half size of serial buffer (minor size IN BYTES)
...
static:
	...
	or		#256, A
	stl		A, 1, *AR5+		; Write to serial buffer
	b	static
static_x:
...
abu_first_half:
	...
	ld		#200, A	; two zero bytes for frame sync
	stl		A, *AR1+
	stl		A, *AR1+
	
	stm		#data_addr, AR2		; data address
...
;	banz	dw_loop, *AR3-
	...
	ld		#3FEh, A	; two 0xFF bytes for frame end
	stl		A, *AR1+
	stl		A, *AR1+
